...
# ========================
# LIMPIEZA DE DATOS ANTERIORES AL INICIO Y AL CIERRE
# ========================
...
equipos_primera_fed = [
    "gimnastica-segoviana-club-de-futbol",
    "real-sociedad-de-futbol-ii",
    "sociedad-deportiva-ponferradina",
    "athletic-club-bilbao-ii",
    "arenteiro",
    "zamora-cf",
    "real-union-club-de-irun",
    "sd-tarazona",
    "ourense-cf",
    "cultural-y-deportiva-leonesa",
    "barakaldo-club-de-futbol",
    "sd-amorebieta",
    "unionistas-de-salamanca",
    "sestao-river-club",
    "club-deportivo-lugo",
    "futbol-club-barcelona-ii",
    "fc-andorra",
    "club-gimnastic-de-tarragona",
    "real-club-celta-de-vigo-ii",
    "ca-osasuna-ii"
]
...
def guardar_partidos():
    ...
    try:
        ...
        # Montaje de overrides para scraping
        overrides = {}
        if override_url_rm:
...
def evaluar_porra():
    try:
        df = pd.read_excel("data/predicciones.xlsx")
        with open("data/resultados.json") as f:
            datos = json.load(f)
        df_filtrado = df.copy()
...
def poll_match(nombre, fecha_var, hora_var, url_var, lbl, convierte_1X2):
    # ----------------------------
    # 1) Integración API para LaLiga:
    url = url_var.get().strip()
    if not url and nombre in ("Real Madrid", "Barcelona"):
        try:
            # Obtenemos todos los partidos de la jornada 1
            matches = fetch_jornada(1)
            # Buscamos el partido por nombre
            for m in matches:
            ...
            # Actualizamos JSON, reevaluamos y subimos a GitHub
        ...
        return
    # ----------------------------
    # 2) Lógica de scraping para override (Ponferradina)
    url = url_var.get().strip()
    if not url:
        lbl.config(text=f"⚠️ {nombre}: sin URL override, omito monitoreo")
        return

    # Espera hasta la hora de fin del partido
    try:
    ...
    # Cargamos JSON una vez (para asegurarnos que override existe)
    with open("data/resultados.json", encoding="utf-8") as f:
        datos = json.load(f)

    # Arrancamos un hilo por partido
    threading.Thread(
        target=poll_match,
        args=("Real Madrid", fecha_rm, hora_rm, url_rm, status_labels['rm'], False),
        daemon=True
    ).start()
    threading.Thread(
        target=poll_match,
        args=("Barcelona",   fecha_bar, hora_bar, url_bar, status_labels['bar'], False),
        daemon=True
    ).start()
    threading.Thread(
        target=poll_match,
        args=("Ponferradina",fecha_ponf, hora_ponf, url_ponf, status_labels['ponf'], True),
        daemon=True
    ).start()
...
tk_vars = {key: tk.StringVar() for key in [
    ...
    'ponf_local', 'ponf_visit', 'fecha_ponf', 'hora_ponf', 'resultado_ponf',
    'url_ponf'
]}
# Aliases para facilitar acceso (necesarios en todo el módulo)
fecha_rm, hora_rm, resultado_rm, url_rm       = (
    tk_vars['fecha_rm'],    tk_vars['hora_rm'],
    tk_vars['resultado_rm'],tk_vars['url_rm']
)
fecha_bar, hora_bar, resultado_bar, url_bar    = (
    tk_vars['fecha_bar'],   tk_vars['hora_bar'],
    tk_vars['resultado_bar'],tk_vars['url_bar']
)
fecha_ponf, hora_ponf, resultado_ponf, url_ponf= (
    tk_vars['fecha_ponf'],  tk_vars['hora_ponf'],
    tk_vars['resultado_ponf'],tk_vars['url_ponf']
)

# Estado global para consultas API
...
def crear_partido(parent, title, equipos, keys, suffix, bg_color):
    ...
    Combobox(frame_top, values=equipos,
             textvariable=tk_vars[keys[0]], width=30, font=font_entry)\
        .pack(side='left', padx=5, expand=True)
    Label(frame_top, text='Visitante', bg=bg_color, font=font_label).pack(side='left', padx=10)
    Combobox(frame_top, values=equipos,
             textvariable=tk_vars[keys[1]], width=30, font=font_entry)\
        .pack(side='left', padx=5, expand=True)
    ...
    DateEntry(frame_mid, textvariable=tk_vars[keys[2]], date_pattern='yyyy-mm-dd',
              font=font_entry, width=12).pack(side='left', padx=5)
    Label(frame_mid, text='Hora fin', bg=bg_color, font=font_label).pack(side='left', padx=10)
    Entry(frame_mid, textvariable=tk_vars[keys[3]], font=font_entry, width=12)\
    ...
    Entry(frame_bot, textvariable=tk_vars[keys[4]], font=font_entry, width=12)\
        .pack(side='left', padx=5)

    # **Campo URL para override manual en cada partido**
    Label(frame_bot, text='URL Soccerway', bg=bg_color, font=font_label).pack(side='left', padx=10)
    Entry(frame_bot, textvariable=tk_vars[f'url_{suffix}'], font=font_entry, width=40)\
        .pack(side='left', padx=5)
...
# Invocar creación de partidos
crear_partido(root, 'Partido 1', equipos_laliga,
              ['rm_local', 'rm_visit', 'fecha_rm', 'hora_rm', 'resultado_rm'],
              'rm', '#FFFBF0')
crear_partido(root, 'Partido 2', equipos_laliga,
              ['bar_local', 'bar_visit', 'fecha_bar', 'hora_bar', 'resultado_bar'],
              'bar', '#F0FFFB')
crear_partido(root, 'Partido 3', equipos_primera_fed,
              ['ponf_local', 'ponf_visit', 'fecha_ponf', 'hora_ponf', 'resultado_ponf'],
              'ponf', '#F0F7FF')
...
def actualizar_semaforos():
    ...
    for suf, lbl in status_labels.items():
        fecha = tk_vars.get(f'fecha_{suf}', tk.StringVar()).get()
        hora = tk_vars.get(f'hora_{suf}', tk.StringVar()).get()
        try:
...
for text, cmd, color in buttons:
    btn = tk.Button(frame_actions, text=text, command=cmd,
                    bg='white', fg=color,
